            ...
            switch (CommandLineData.Global.Action)
            {
                case CommandLineData.Action.GenerateCertificate:
                    ...
                    if (!String.IsNullOrEmpty(CommandLineData.Sign.NewCertificate.CopyFrom))
                    {
                        ...
                        {
                            // "Is it signed?" is the wrong question and sends people
                            // in circles: notepad.exe IS signed. CreateFromSignedFile
                            // reads an EMBEDDED Authenticode signature, and most
                            // Windows system binaries are catalog-signed instead, so
                            // they carry none.
                            Logger.Error($"Could not read an embedded Authenticode certificate from: {CommandLineData.Sign.NewCertificate.CopyFrom}");
                            Logger.Error("  Only an EMBEDDED signature can be copied from. Most Windows system");
                            Logger.Error("  binaries are catalog-signed and carry no certificate of their own.");
                            Logger.Error("  Find one that does:");
                            Logger.Error("    Get-ChildItem 'C:\\Program Files\\*' -Recurse -Filter *.exe -EA 0 |");
                            Logger.Error("      ForEach-Object { Get-AuthenticodeSignature $_ } |");
                            Logger.Error("      Where-Object SignatureType -eq 'Embedded' | Select-Object -First 5 Path");
                            Logger.Error($"  Underlying error: {ex.GetType().Name}: {ex.Message}");
                            return false;
                        }
                    }
