    ...
    class SharpView : BaseProfile
    {
        ...
        public override async Task<Solution> After(Solution solution, Project project)
        {
            ...
            if (CommandLineData.CSharp.Rename.Namespaces)
            {
                ...
                string sharpViewNamespace = RequireMapping(DataCollector.Mapping.Namespaces, "SharpView", "namespace");
                if (sharpViewNamespace == null)
                {
                    return solution;
                }
                ...
                {
                    { "AssemblyName", sharpViewNamespace },
                    { "RootNamespace", sharpViewNamespace },
                };
            ...
            if (CommandLineData.CSharp.Rename.Namespaces || CommandLineData.CSharp.Rename.Classes)
            {
                ...
                if (CommandLineData.CSharp.Rename.Namespaces)
                {
                    string ns = RequireMapping(DataCollector.Mapping.Namespaces, "SharpView", "namespace");
                    if (ns == null)
                    {
                        return solution;
                    }
                ...
                {
                    string cls = RequireMapping(DataCollector.Mapping.Classes, "Program", "class");
                    if (cls == null)
                    {
                        return solution;
                    }
        ...
        protected async Task<Solution> RewriteCommandLine(Solution solution, Project project)
        {
            ...
            Document csProgram = RequireDocument(project, "Program.cs");
            if (csProgram == null)
            {
                return solution;
            }
