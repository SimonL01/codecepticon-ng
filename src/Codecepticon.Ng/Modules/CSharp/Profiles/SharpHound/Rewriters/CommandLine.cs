    ...
    class CommandLine : CSharpSyntaxRewriter
    {
        ...
        public override SyntaxNode VisitLiteralExpression(LiteralExpressionSyntax node)
        {
            if (Helper.IsOptionParent(node))
            {
                ...
                {
                    // Same reasoning as Seatbelt's "all": if the enum member was
                    // never renamed, rewriting the literal would point at a name
                    // that does not exist.
                    if (!DataCollector.Mapping.Enums.TryGetValue(value, out newValue))
                    {
                        Logger.Warning($"SharpHound: the '{value}' enum member was not renamed - leaving the literal as-is.");
