...
namespace Codecepticon.Modules.CSharp.Profiles.SharpHound
{
    class SharpHound : BaseProfile
    {
        ...
        public override async Task<Solution> Final(Solution solution, Project project)
        {
            ...
            // Was a SharpHound-specific workaround for dotnet/roslyn#36781.
            // It is not SharpHound-specific - every project hits it - so it now
            // lives in BaseProfile, which also handles the SDK-style case.
        ...
        protected async Task<Solution> RewriteCommandLine(Solution solution, Project project)
        {
            ...
            Document csOptions = RequireDocument(project, "Options.cs");
            if (csOptions == null)
            {
                return solution;
            }
