...
namespace Codecepticon.Modules.CSharp.CommandLine
{
    class CSharpCommandLine : CommandLineManager
    {
        public CSharpCommandLine(string[] args) : base(args)
        {
            ...
            // A flag absent from this table is never parsed, whatever the switch
            // below does with it. "switch" means it takes no value.
            Arguments = new Dictionary<string, string>
            {
                ...
                { "profile", "" },
                { "precompile", "switch" },
                { "allow-multi-project", "switch" },
                { "verify", "switch" },
                { "backup", "switch" },
                { "backup-path", "" },
                { "no-reflection-scan", "switch" }
            };
            ...
            foreach (KeyValuePair<string, string> argument in arguments)
            {
                ...
                {   
                    ...
                    case "verify":
                        if (argument.Value.ToLower() != "false")
                        {
                            CommandLineData.CSharp.Compilation.Verify = (argument.Value.Length > 0);
                        }
                        break;
                    case "backup":
                        if (argument.Value.ToLower() != "false")
                        {
                            CommandLineData.CSharp.Backup = (argument.Value.Length > 0);
                        }
                        break;
                    case "backup-path":
                        CommandLineData.CSharp.BackupPath = argument.Value;
                        // A path implies the intent; requiring both flags is a trap.
                        CommandLineData.CSharp.Backup = true;
                        break;
                    case "no-reflection-scan":
                        if (argument.Value.ToLower() != "false")
                        {
                            CommandLineData.CSharp.SkipReflectionScan = (argument.Value.Length > 0);
                        }
                        break;
                    case "allow-multi-project":
                        if (argument.Value.ToLower() != "false")
                        {
                            CommandLineData.CSharp.AllowMultiProject = (argument.Value.Length > 0);
                        }
                        break;
                    //default:
