...
namespace Codecepticon.Modules.CSharp.Rewriters
{
    class Strings : CSharpSyntaxRewriter
    {
        ...
        {
            // Alignment and format clauses used to disqualify the whole string:
            // $"loot {n:N2}" was skipped entirely and shipped in the clear, which
            // is the opposite of what a reader expects from --string-rewrite.
            // They translate directly instead - interpolation and composite
            // format strings share the {index[,alignment][:format]} syntax, so
            // $"{x,10:N2}" becomes String.Format("{0,10:N2}", x).
            string format = "";
            string vars = "";
            int varCount = 0;
            ...
            foreach (SyntaxNode n in node.ChildNodes())
            {
                switch (n.Kind())
                {
                    ...
                    case SyntaxKind.Interpolation:
                        InterpolationSyntax interpolation = (InterpolationSyntax)n;

                        if (vars.Length > 0) { vars += ","; }
                        vars += interpolation.Expression.ToString();

                        string alignment = interpolation.AlignmentClause == null
                            ? ""
                            : "," + interpolation.AlignmentClause.Value.ToString();

                        string clause = interpolation.FormatClause == null
                            ? ""
                            : ":" + interpolation.FormatClause.FormatStringToken.ValueText;

                        format += $"{{{varCount}{alignment}{clause}}}";
                        varCount++;
